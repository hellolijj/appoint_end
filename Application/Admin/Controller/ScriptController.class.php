...
class ScriptController extends Controller {
    ...
    public function load_user_back_4() {
        // die('user_back_3已经入库，无需再操作');

        // 将新加入的数据表 添加进入到 user_back里，以passport为键值，若重复则，不该，如新的，则添加

        $NEW_USER = M('user_back_4');
        $new_data = $NEW_USER->select();
        $OLD_USER = M('user_back');
        ...
        foreach ($new_data as $_new_data) {
            ...
            unset($_new_data['id']);
            if (is_null($_new_data['first_name'])) {
                $_new_data['first_name'] = '';
            } else {
                $_new_data['first_name'] = trim($_new_data['first_name']);
            }


            if (is_null($_new_data['last_name'])) {
                $_new_data['last_name'] = '';
            } else {
                $_new_data['last_name'] = trim($_new_data['last_name']);
            }
