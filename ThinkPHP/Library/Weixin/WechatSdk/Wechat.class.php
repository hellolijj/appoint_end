...
class Wechat {
    ...
    const MERCHANT_ORDER_GETBYID = '/merchant/order/getbyid?';//以页面为维度的数据统计接口
    ///微信小店相关接口
    const MERCHANT_ORDER_GETBYFILTER = '/merchant/order/getbyfilter?';//根据订单ID获取订单详情
    const MERCHANT_ORDER_SETDELIVERY = '/merchant/order/setdelivery?';//根据订单状态/创建时间获取订单详情
    const MERCHANT_ORDER_CLOSE = '/merchant/order/close?';//设置订单发货信息
    static $DATACUBE_URL_ARR = array(        //用户分析
        'user' => array('summary' => '/datacube/getusersummary?',        //获取用户增减数据（getusersummary）
    ...
    static function json_encode ($arr)
    {
        ...
        foreach ($arr as $key => $value) {
            if (is_array($value)) { //Custom handling for arrays
                if ($is_list)
                    $parts [] = self::json_encode($value); /* :RECURSION: */ else
    ...
    public function Message ($msg = '', $append = FALSE)
    {
        if (is_null($msg)) {
            ...
            if ($append)
                $this->_msg = array_merge($this->_msg, $msg); else
                $this->_msg = $msg;
            return $this->_msg;
        } else {
            return $this->_msg;
    ...
    public function getRevFrom ()
    {
        if (isset($this->_receive['FromUserName']))
            return $this->_receive['FromUserName']; else
            return FALSE;
    }
    ...
    public function getRevTo ()
    {
        if (isset($this->_receive['ToUserName']))
            return $this->_receive['ToUserName']; else
            return FALSE;
    }
    ...
    public function getRevType ()
    {
        if (isset($this->_receive['MsgType']))
            return $this->_receive['MsgType']; else
            return FALSE;
    }
    ...
    public function getRevID ()
    {
        if (isset($this->_receive['MsgId']))
            return $this->_receive['MsgId']; else
            return FALSE;
    }
    ...
    public function getRevCtime ()
    {
        if (isset($this->_receive['CreateTime']))
            return $this->_receive['CreateTime']; else
            return FALSE;
    }
    ...
    public function getRevContent ()
    {
        if (isset($this->_receive['Content']))
            return $this->_receive['Content']; elseif (isset($this->_receive['Recognition'])) //获取语音识别文字内容，需申请开通
            return $this->_receive['Recognition']; else
            return FALSE;
    }
    ...
    public function getRevPic ()
    {
        if (isset($this->_receive['PicUrl']))
            return array('mediaid' => $this->_receive['MediaId'], 'picurl' => (string)$this->_receive['PicUrl'],    //防止picurl为空导致解析出错
            ); else
            return FALSE;
    }
    ...
    public function getRevCardPass ()
    {
        if (isset($this->_receive['CardId']))
            return $this->_receive['CardId']; else
            return FALSE;
    }
    ...
    public function getRevOrderId ()
    {
        if (isset($this->_receive['OrderId']))     //订单 ID
            return $this->_receive['OrderId']; else
            return FALSE;
    }
    ...
    {
        $FuncFlag = $this->_funcflag ? 1 : 0;
        $msg = array('ToUserName' => $this->getRevFrom(), 'FromUserName' => $this->getRevTo(), 'CreateTime' => time(), 'MsgType' => self::MSGTYPE_MUSIC, 'Music' => array('Title' => $title, 'Description' => $desc, 'MusicUrl' => $musicurl, 'HQMusicUrl' => $hgmusicurl), 'FuncFlag' => $FuncFlag);
        if ($thumbmediaid) {
    ...
    public function getRevOrderSkuInfo ()
    {
        if (isset($this->_receive['SkuInfo']))     //订单 SkuInfo
            return $this->parseSkuInfo($this->_receive['SkuInfo']); else
            return FALSE;
    }
