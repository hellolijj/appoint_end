...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader {
                                ...
                                foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
                                    ...
                                    if ($filterColumn->dynamicFilter) {
                                        ...
                                        foreach ($filterColumn->dynamicFilter as $filterRule) {
                                            $column->createRule()->setRule(//    Operator is undefined, but always treated as EQUAL
                                                NULL, (string)$filterRule["val"], (string)$filterRule["type"])->setRuleType(PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
    ...
    private function getFromZipArchive ($archive, $fileName = '')
    {
        ...
        if ($contents === FALSE) {
            $contents = $archive->getFromIndex($archive->locateName(substr($fileName, 1), ZIPARCHIVE::FL_NOCASE));
        }
