...
define(['jquery', 'admin.plugs'], function () {
    ...
    this.$body.on('click', '[data-modal]', function () {
        return $.form.modal($(this).attr('data-modal'), 'open_type=modal', $(this).attr('data-title') || '编辑', $(this).attr('data-area')||'500px', $(this).attr('data-btn'));
    });
    ...
    /*! 注册 data-switch 事件行为 */
    this.$body.on('click', '[data-switch]', function () {
        $.Table.switch.call(this);
    });

    /*! 注册 data-edit 事件行为 */
    this.$body.on('click', '[data-edit]', function () {
        $.Table.edit.call(this);
    });
    ...
    this.$body.on('click', '[data-update]', function () {
        var msg = $(this).data('msg')||'确定要操作这些数据吗';
